...
import { CreditCard, Loader2, CheckCircle, AlertCircle, Usb } from "lucide-react"
...
export default function RFIDWriter({ voterHash, voterName, onSuccess }: RFIDWriterProps) {
  const { connect, writeTag, disconnect, status, error, isConnected } = useRFID()
  ...
  return (
    <div className="flex flex-col items-center gap-6">
      ...
      <div className="w-full p-3 rounded-xl bg-white/5 border border-white/10">
        <p className="text-xs text-gray-500 mb-1">Voter Hash (writing to card)</p>
      ...
      <div className="flex flex-col gap-3 w-full">
        {!isConnected ? (
          ...
          >
            {isWriting ? (
              <><Loader2 size={16} className="animate-spin" />Writing to card...</>
            ) : (
              <><CreditCard size={16} />Write to Card</>
            )}
          </button>
        ) : (
...
function writerLabel(status: string, written: boolean): string {
  ...
  switch (status) {
    ...
    case "connected":         return "ready to write"
