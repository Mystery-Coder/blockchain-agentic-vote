...
function getDeploymentInfo(): DeploymentInfo {
	const deploymentPath = path.join(process.cwd(), "data", "deployment.json");
...
export function getRelayerSigner(): ethers.Wallet {
	if (!globalForBlockchain.bcSigner) {
		...
		if (!privateKey) {
			throw new Error(
				"RELAYER_PRIVATE_KEY environment variable is not set",
			);
		}
		globalForBlockchain.bcSigner = new ethers.Wallet(
			privateKey,
			getProvider(),
		);
	}
...
export function getWritableContract(): ethers.Contract {
	const { contractAddress } = getDeploymentInfo();
	return new ethers.Contract(contractAddress, BALLOT_ABI, getRelayerSigner());
}
...
export async function hasVoterVotedOnChain(
	...
	return await contract.hasVoterVoted(normalizeBytes32(voterHash));
}
...
export function normalizeBytes32(value: string): string {
	...
	const hexOnly = /^[0-9a-fA-F]{64}$/;
	if (!hexOnly.test(trimmed)) {
		throw new Error(
			"Invalid voter hash format. Expected 32-byte hex string.",
		);
	}
