 ...
 * 2. Separate voterHashes[] and candidateIds[]
 * 3. Fisher-Yates shuffle candidateIds[] (breaking the link)
 * 4. Submit markVoters(voterHashes[]) to chain
 * 5. Submit recordVotes(shuffledCandidateIds[]) to chain
 */
export async function processBatch(
  ...
  const uniqueVotes = votes.filter((v) => {
    if (seenHashes.has(v.voterHash)) {
      console.warn(
        `[Batcher] Duplicate voterHash detected and removed: ${v.voterHash.slice(0, 10)}...`
      );
      return false;
    }
    seenHashes.add(v.voterHash);
    return true;
  });

  if (uniqueVotes.length === 0) {
    return { success: true, message: "No unique votes in buffer.", voterCount: 0 };
  }

  // 3. Separate arrays
  const voterHashes = uniqueVotes.map((v) => v.voterHash);
